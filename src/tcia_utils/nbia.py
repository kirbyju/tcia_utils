...
def downloadSeries(series_data: Union[str, pd.DataFrame, List[str]],
    ...
    if uids_to_download:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm(concurrent.futures.as_completed(future_to_uid), total=len(uids_to_download), desc="Downloading series"):
    ...
    if manifestDF is not None:
        ...
        if uids_for_metadata:
            _log.info(f"Fetching metadata for {len(uids_for_metadata)} series...")
            # The V2 API does not have a batch metadata endpoint, so we loop.
            for seriesUID in tqdm(uids_for_metadata, desc="Fetching metadata"):
                metadata = getSeriesMetadata(seriesUID, api_url=api_url)
                if metadata:
